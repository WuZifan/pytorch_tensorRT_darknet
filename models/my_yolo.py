...
class MyTinyYolov3(nn.Module):
    anchors = [
        [(81, 82), (135, 169), (344, 319)],
        [(10, 14), (23, 27), (37, 58)]
    ]
    def __init__(self,num_class = 1,img_size = 416,is_trained=False):
        super(MyTinyYolov3, self).__init__()
        self.is_trained = is_trained
        self.img_dim=img_size
        self.downsample = nn.Sequential(
            self._create_conv(inchannel=3, outchannel=16, kernel_size=3, stride=1, padding=1,pool=True),
            self._create_conv(inchannel=16, outchannel=32, kernel_size=3, stride=1, padding=1, pool=True),
            self._create_conv(inchannel=32, outchannel=64, kernel_size=3, stride=1, padding=1, pool=True),
            self._create_conv(inchannel=64, outchannel=128, kernel_size=3, stride=1, padding=1, pool=True),
            self._create_conv(inchannel=128, outchannel=256, kernel_size=3, stride=1, padding=1,pool=False),
        )
        ...
        self.b1 =nn.Sequential(
            ...
            self._create_conv(inchannel=256, outchannel=512, kernel_size=3, stride=1, padding=1,pool=True,pool_stride=1),
            self._create_conv(inchannel=512, outchannel=1024, kernel_size=3, stride=1, padding=1, pool=False),
            self._create_conv(inchannel=1024, outchannel=256, kernel_size=1, stride=1, padding=0, pool=False),
        )

        self.header1 = nn.Sequential(
            self._create_conv(inchannel=256, outchannel=512, kernel_size=3, stride=1, padding=1, pool=False),
            nn.Conv2d(in_channels=512,out_channels=3*(1+4+num_class),kernel_size=1,stride=1,padding=0)
        )

        self.upsample= nn.Sequential(
            self._create_conv(inchannel=256, outchannel=128, kernel_size=1, stride=1, padding=0, pool=False),
            self._create_upsample(128)
        )


        self.header2 = nn.Sequential(
            self._create_conv(inchannel=384, outchannel=256, kernel_size=3, stride=1, padding=1, pool=False),
            nn.Conv2d(in_channels=256,out_channels=3*(1+4+num_class),kernel_size=1,stride=1,padding=0)
        )
        ...
        if self.is_trained:
            ...
            self.yolo_layer1 = YOLOLayer(self.anchors[0],2,self.img_dim)
            self.yolo_layer2 = YOLOLayer(self.anchors[1],2,self.img_dim)
            self.yolo_layers = [self.yolo_layer1, self.yolo_layer2]
            self.seen=0
    ...
    def forward(self,x,targets=None):
        ...
        if self.is_trained:
            self.seen+=1
            yolo_output1, loss1 = self.yolo_layer1(o1, targets, self.img_dim)
            yolo_output2, loss2 = self.yolo_layer2(o2, targets, self.img_dim)

            yolo_outputs = to_cpu(torch.cat([yolo_output1, yolo_output2], 1))
    ...
    def _create_conv(self,inchannel,outchannel,kernel_size,stride,padding,pool=False,pool_stride=2):
        model = nn.Sequential(
            nn.Conv2d(in_channels=inchannel,
                      out_channels=outchannel,
                      kernel_size=kernel_size,
        ...
        if pool:
            model.add_module('pool',nn.MaxPool2d(kernel_size=2,stride=pool_stride))
        return model

if __name__ == '__main__':

    model = MyTinyYolov3()
    test_input = torch.rand([2,3,352,352])


    o1,o2 = model(test_input)
    ...
    # model = MyYolov3_CV()
    # model.save_darknet_weights('../weights/a_test.weights')
    # weights_path = '../weights/yolov3-myyolov3_99_0.96_warehouse.pth'
